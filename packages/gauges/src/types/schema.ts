...
export class GaugeInjector extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save GaugeInjector entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type GaugeInjector must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("GaugeInjector", id.toString(), this);
    }
  }

  static loadInBlock(id: string): GaugeInjector | null {
    return changetype<GaugeInjector | null>(
      store.get_in_block("GaugeInjector", id)
    );
  }

  static load(id: string): GaugeInjector | null {
    return changetype<GaugeInjector | null>(store.get("GaugeInjector", id));
  }
